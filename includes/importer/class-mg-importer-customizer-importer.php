<?php
/**
 * Class for the customizer importer used in the Mirrorgrid Demo Importer plugin.
 *
...
class MG_Importer_Customizer_Importer {
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new WP_Error(
				...
				sprintf(
					esc_html__( 'The customizer import file is missing! File path: %s', 'mirrorgrid-demo-importer' ),
					$import_file_path
				)
			);
		}
		...
		// Get the upload data.
		$raw  = MG_Helpers::data_from_file( $import_file_path );
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'mirrorgrid-demo-importer' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'mirrorgrid-demo-importer' )
			);
		}

		// Import images.
		if ( apply_filters( 'mirrorgrid-demo-importer/customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new MG_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
