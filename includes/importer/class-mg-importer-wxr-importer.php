...
class MG_Importer_WXR_Importer {
	/**
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'mirrorgrid-demo-importer' ) );
		}
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'mirrorgrid-demo-importer' ),
							$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'mirrorgrid-demo-importer' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'mirrorgrid-demo-importer' ) );
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'mirrorgrid-demo-importer' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'mirrorgrid-demo-importer' ),
				$data['post_title'],
				$data['post_type']
			) );
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__( '%s "%s" already exists.', 'mirrorgrid-demo-importer' ),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice( sprintf(
					__( 'Skipping attachment "%s", fetching attachments disabled', 'mirrorgrid-demo-importer' ),
					$data['post_title']
				) );
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%s" (%s)', 'mirrorgrid-demo-importer' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'mirrorgrid-demo-importer' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'mirrorgrid-demo-importer' ),
			$original_id,
			$post_id
		) );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'mirrorgrid-demo-importer' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"', 'mirrorgrid-demo-importer' ),
				$userdata['user_login']
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"', 'mirrorgrid-demo-importer' ),
			$userdata['user_login']
		) );
		$this->logger->debug( sprintf(
			__( 'User %d remapped to %d', 'mirrorgrid-demo-importer' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->debug( sprintf(
			__( 'Term %d remapped to %d', 'mirrorgrid-demo-importer' ),
			$original_id,
			$term_id
		) );
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'mirrorgrid-demo-importer' ),
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			...
			return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'mirrorgrid-demo-importer' ) );
		}

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );

			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'mirrorgrid-demo-importer' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'mirrorgrid-demo-importer' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			...
			// for when debug logging is off
				__( 'Running post-processing for post %d', 'mirrorgrid-demo-importer' ),
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the post parent for "%s" (post #%d)', 'mirrorgrid-demo-importer' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with parent %d, but could not be found', 'mirrorgrid-demo-importer' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for "%s" (post #%d)', 'mirrorgrid-demo-importer' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with author "%s", but could not be found', 'mirrorgrid-demo-importer' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.', 'mirrorgrid-demo-importer' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update "%s" (post #%d) with mapped data', 'mirrorgrid-demo-importer' ),
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning( sprintf(
				__( 'Could not find the menu object for "%s" (post #%d)', 'mirrorgrid-demo-importer' ),
				get_the_title( $post_id ),
				$post_id
			) );
			$this->logger->debug( sprintf(
				__( 'Post %d was imported with object "%d" of type "%s", but could not be found', 'mirrorgrid-demo-importer' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the comment parent for comment #%d', 'mirrorgrid-demo-importer' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with parent %d, but could not be found', 'mirrorgrid-demo-importer' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for comment #%d', 'mirrorgrid-demo-importer' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with author %d, but could not be found', 'mirrorgrid-demo-importer' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update comment #%d with mapped data', 'mirrorgrid-demo-importer' ),
					$comment_id
				) );
				continue;
			}
