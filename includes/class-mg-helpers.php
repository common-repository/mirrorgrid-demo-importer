...
class MG_Helpers {
	...
	public static function download_import_files( $import_file_info, $start_date = '' ) {
		...
		$upload_path      = apply_filters( 'mirrorgrid-demo-importer/upload_file_path', trailingslashit( $upload_dir['path'] ) );
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			if ( file_exists( $import_file_info['local_import_file'] ) ) {
				...
				return new WP_Error(
					...
					sprintf(
						__( '"import_file_url" or "local_import_file" for %s%s%s are not defined!', 'mirrorgrid-demo-importer' ),
						'<strong>',
			...
			$demo_import_file_path = $upload_path . apply_filters( 'mirrorgrid-demo-importer/downloaded_content_file_prefix', 'demo-content-import-file_' ) . $start_date . apply_filters( 'mirrorgrid-demo-importer/downloaded_content_file_suffix_and_file_extension', '.xml' );
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$import_widgets_file_path = $upload_path . apply_filters( 'mirrorgrid-demo-importer/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . $start_date . apply_filters( 'mirrorgrid-demo-importer/downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$import_customizer_file_path = $upload_path . apply_filters( 'mirrorgrid-demo-importer/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . $start_date . apply_filters( 'mirrorgrid-demo-importer/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
	...
	private static function get_content_from_url( $url, $file_name = 'Import file' ) {
		...
		if ( empty( $url ) ) {
			return new WP_Error(
				'url_not_defined',
				sprintf(
					__( 'URL for %s%s%s file is not defined!', 'mirrorgrid-demo-importer' ),
					'<strong>',
					$file_name,
					'</strong>'
				)
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => apply_filters( 'mirrorgrid-demo-importer/timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new WP_Error(
				'file_fetching_error',
				sprintf(
					__( 'An error occurred while fetching %s%s%s file from the server!%sReason: %s - %s.', 'mirrorgrid-demo-importer' ),
					'<strong>',
					$file_name,
					'</strong>',
					'<br>',
				...
				apply_filters( 'mirrorgrid-demo-importer/message_after_file_fetching_error', '' )
			);
		}
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'mirrorgrid-demo-importer' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'mirrorgrid-demo-importer' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'mirrorgrid-demo-importer' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters( 'mirrorgrid-demo-importer/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'Mirrorgrid Demo Import', 'mirrorgrid-demo-importer' ),
				'menu_title'  => esc_html__( 'Import Demo Data', 'mirrorgrid-demo-importer' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'mirrorgrid-demo-importer' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'mirrorgrid-demo-importer' )
			);
		}
	...
	public static function get_log_path( $start_date = '' ) {
		...
		$log_path = $upload_path . apply_filters( 'mirrorgrid-demo-importer/log_file_prefix', 'log_file_' ) . $start_date . apply_filters( 'mirrorgrid-demo-importer/log_file_suffix_and_file_extension', '.txt' );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$filetype  = wp_check_filetype( basename( $log_path ), apply_filters( 'mirrorgrid-demo-importer/file_mimes', $log_mimes ) );

		// Prepare an array of post data for the attachment.
		$attachment = array(
			'guid'           => self::get_log_url( $log_path ),
			'post_mime_type' => $filetype['type'],
			'post_title'     => apply_filters( 'mirrorgrid-demo-importer/attachment_prefix', esc_html__( 'Mirrorgrid Demo Import - ', 'mirrorgrid-demo-importer' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function get_log_url( $log_path ) {
		...
		$upload_dir = wp_upload_dir();
		$upload_url = apply_filters( 'mirrorgrid-demo-importer/upload_file_url', trailingslashit( $upload_dir['url'] ) );
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'mirrorgrid-demo-importer' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ) {
			...
			self::log_error_and_send_ajax_response(
				__( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', 'mirrorgrid-demo-importer' ),
				$log_file_path,
				esc_html__( 'Upload files', 'mirrorgrid-demo-importer' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', 'mirrorgrid-demo-importer' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', 'mirrorgrid-demo-importer' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'mirrorgrid-demo-importer' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'mirrorgrid-demo-importer' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		return PHP_EOL .
		       sprintf(
			       __( 'Initial max execution time = %s', 'mirrorgrid-demo-importer' ),
			       ini_get( 'max_execution_time' )
		       ) . PHP_EOL .
		       sprintf(
			       __( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s', 'mirrorgrid-demo-importer' ),
			       PHP_EOL,
			       get_site_url(),
			       $selected_import_files['content'],
			       empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'mirrorgrid-demo-importer' ) : $selected_import_files['widgets'],
			       empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'mirrorgrid-demo-importer' ) : $selected_import_files['customizer']
		       );
	}
